...
logo = (f''' 
        \033[1m CREATED BY BISHESH 
        \033[1m MODIFIED BY JUDE{c}「{red}v•2{c}」{r} \n''')
...
def linktradio(post_link):
    def extract_postid_from_link(post_link):
        ...
        pattern = r'https://www\.facebook\.com/\d+/groups/(\d+)/?'
...
def overview():
    ...
    print(f"  {r}     Number Of Accounts: {r}{total_accounts}{r} | Number Of Pages: {c}{total_pages} {r}")
    print(f'{c}  ══════════════════════════════════════════════════{wh}')
...
def ip():
    ...
    print(f" {c}  「{w}State{c}」{w}:{c}Paid_lifetime")
    ...
    print(f"{c}   「{w}s-key{c}」{w}:{c}No need S-key {r}")
    linex()
